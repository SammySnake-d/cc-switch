...
impl RequestForwarder {
    ...
    async fn forward(
        ...
        let mapped_different = match (&orig_model, &final_model) {
            (Some(orig), Some(mapped)) if orig != mapped => true,
            _ => false,
        };

        if mapped_different {
            let orig = orig_model.as_ref().unwrap();
            let mapped = final_model.as_ref().unwrap();
